...
export async function uploadCSV(file: File) {
  ...
  const res = await fetch("/etl/upload", { method: "POST", body: form });
  if (!res.ok) {
...
export async function getPropiedades(limit = 100, offset = 0) {
  const res = await fetch(`/etl/propiedades?limit=${limit}&offset=${offset}`);
...
export async function actualizarClima(nombres?: string[]) {
  const params = nombres?.length
    ...
    : "";
  const res = await fetch(`/clima/actualizar${params}`, { method: "POST" });
  if (!res.ok) throw new Error(res.statusText);
  return res.json() as Promise<{
    actualizados: number;
    errores: number;
